...
$(\text{3)设方程}x\text{y}-z\text{ln y}+\mathrm{e}^{x\pi}=1,\text{存在点}(0,1,1)\text{的一}\text{个邻域},\text{在此邻域内该方程}(\text{)}.$
...
$\text{(5)设 }f(x,y)=\mathrm{e}^{2x}\left(x+y^2+2y\right),\text{则}f(x,y)\text{ 在点 }P\left(\frac{1}{2},-1\right)\text{处}(\quad).$
...
$(7)\text{设}f(x,y)=\frac{\mathrm{e}^{x}}{x-y},\text{则}(\quad).$​
...
$(2)\lim_{\substack{x\to\infty\\y\to\infty}}\frac{x+y}{x^2-xy+y^2}=.$

$(3)\lim_{\substack{x\to\infty\\y\to0}}\left(1-\frac1{2x}\right)^{\frac{x^2}{x+y}}=$

$(4)\text{ 设 }z=(1+xy)^y,\text{则 }\mathrm{d}z\bigg|_{(1,1)}=$

$(5)\text{设函数}f(x,y)\text{可微,且}f(1,2)=2,f'_x(1,2)~=~3,f'_y(1,2)~=~4,F(x)~=~\\f[x,f(x,2x)],\text{则}F'(1)~=~$
...
$(\text{13)设}z(x,y)\text{的全微分}\mathrm{d}z=(x^2+2xy-y^2)\mathrm{d}x+(x^2-2xy-y^2)\mathrm{d}y,\text{则}z(x,y)=$

$(\text{14)设}z=z(x,y)\text{由方程}z+\ln z-\int_y^x\mathrm{e}^{-t^2}\mathrm{d}t=0\text{确定},\text{则}\frac{\partial^2z}{\partial x\partial y}=$
...
$\text{(17)曲线 L: }\begin{cases}x^2+y^2=10,\\x^2+z^2=10,\end{cases}\text{在点P(3,1,1)处的切线方程为(),法线方程为()}$

$\text{(18)设}\frac{(x+ky)\mathrm{d}x+y\mathrm{d}y}{(x+y)^2}\text{为某二元函数的全微分},\text{则}k=$

$\text{(19)设 }f(x,y)\text{ 有连续偏导数},\text{在 }P(1,-2)\text{处有 }f^{\prime}(1,-2)=1,f^{\prime}(1,-2)=-1,\text{则}f(x,y)\text{在}P(1,-2)\text{处增加最快的方向为()}$
...
$(5)\text{ 求双曲线 }xy=4\text{ 与直线 }2x+y=1\text{ 之间的最短距离}.$

$(6)\text{求函数}z=x^3-3x^2-3y^2\text{在闭区域}D{:}x^2+y^2\leqslant16\text{ 上的最大值}.$

$\text{(7)设椭圆}\begin{cases}z=x^2+y^2,\\x+y+z=4,\end{cases}\text{上的点}(x,y,z)\text{到原点的距离为}d\text{求其最值以使得}d\text{取最大和最小的点}$​​
...
(8) $\frac{G_t^{\prime}f_x^{\prime}-G_x^{\prime}f_t^{\prime}}{G_y^{\prime}f_t^{\prime}+G_t^{\prime}}$
